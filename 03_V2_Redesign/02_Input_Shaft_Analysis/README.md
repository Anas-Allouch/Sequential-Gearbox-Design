...
safety factor is Goodman.  
In some places, ASME elliptic criteria might produce low values, and that's completely normal when the moment values at that location are low. And that weird behavior is one of the reasons why Goodman is the criteria to follow since it is conservative in all cases.  
